    ...
    for batch_idx in range(num_batches):
        ...
        for model in models:
            ...
            if result and 'qa_list' in result and 'scores' in result:
                ...
                print(f"✅ 模型 {model} 批次 {batch_idx + 1} 完成")
            else:
        ...
        for dim in dimensions:
            ...
            if len(orig_scores) >= 2:
                ...
                new_orig_volatility.append({
                    ...
                    '变异系数': orig_vol['cv'],
                    '极差': orig_vol['range'],
                    '最小值': round(np.min(orig_scores), 4),
                    '最大值': round(np.max(orig_scores), 4)
                })
            ...
            if len(final_scores) >= 2:
                ...
                new_final_volatility.append({
                    ...
                    '变异系数': final_vol['cv'],
                    '极差': final_vol['range'],
                    '最小值': round(np.min(final_scores), 4),
                    '最大值': round(np.max(final_scores), 4)
                })
    ...
    for model in new_results["original_results"].keys():
        ...
        if len(batch_scores) >= 2:
            ...
            new_orig_model_volatility.append({
                ...
                '最小值': round(np.min(batch_scores), 4),
                '最大值': round(np.max(batch_scores), 4)
            })
