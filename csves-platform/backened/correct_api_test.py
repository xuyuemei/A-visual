...
def call_correct_api(question, model_id):
    """使用正确的模型ID调用API"""
    try:
        ...
        questions_data = {
            'headers': ['问题', '模型'],
            'data': [[question, f'model_{model_id}']]
        }
        ...
        if response.status_code == 200:
            try:
                ...
                print(f"⚠️ 模型ID {model_id} 返回非JSON格式")
                # 尝试解析celldata格式
                try:
                    result = response.json()
                    return result
...
def parse_evaluation_result(result, question, model_id):
    """解析评估结果"""
    try:
...
def run_correct_api_test(num_questions=1, model_ids=["1", "2", "3", "4"]):
    """运行正确的API测试"""
    print("🧪 开始正确API质量对比测试")
    print("=" * 80)
    ...
    for model_id in model_ids:
        ...
        for i, question in enumerate(orig_test, 1):
            ...
            # 调用API
            result = call_correct_api(question, model_id)
            if result:
                parsed = parse_evaluation_result(result, question, model_id)
                if parsed:
...
def create_correct_api_excel_report(results):
    """创建正确API评估Excel报告"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    excel_file = f"/data/hlt/A-visual/mark/correct_api_evaluation_report_{timestamp}.xlsx"
    
    print(f"\n📊 生成正确API评估Excel报告: {excel_file}")
    ...
    print(f"✅ 正确API评估Excel报告已生成: {excel_file}")
    return excel_file
...
def main():
    ...
    print("🧪 正确API质量对比测试工具")
    print("=" * 80)
    print("🤖 使用正确的模型ID调用后端4个模型进行评估")
    print("📊 对比原始90个问题 vs 最终499个问题")
    print("⚠️  真实API调用，需要较长时间")
    print()
    
    # 运行测试
    results = run_correct_api_test(
    ...
    if results:
        # 生成Excel报告
        excel_file = create_correct_api_excel_report(results)
